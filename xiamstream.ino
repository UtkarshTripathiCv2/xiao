...
// --- 2. XIAO ESP32S3 Sense Pin Definitions ---
...
// --- 3. Stream Handler (Sends the video data) ---
...
void setup() {
  ...
  config.frame_size = FRAMESIZE_QVGA; // Small resolution for stability
  ...
  if (httpd_start(&server, &server_config) == ESP_OK) httpd_register_uri_handler(server, &stream_uri);
